...
{
private:
	std::vector<Object*>	create_objects;		// 生成を行うオブジェクトリスト
	std::vector<Object*>	game_objects_list;	// 更新と描画を行うオブジェクトリスト
	std::vector<Object*>	destroy_objects;	// 破棄を行うオブジェクトリスト
...
public:
	...
	void CheckCreateObject()
	{
		...
		{
			for (Object* obj : create_objects)
			{
				std::vector<Object*>::iterator itr = game_objects_list.begin();	// オブジェクトリストの先頭
	...
	void CheckDestroyObject()
	{
		...
		{
			for (Object* obj : destroy_objects)
			{
				std::vector<Object*>::iterator itr = game_objects_list.begin();	// オブジェクトリストの先頭
				// リストの末尾になるまで走査する
	...
	const std::vector<Object*>& GetObjectsList() const
	{
	...
	template <class T>
	T* CreateObject(const GameLib::Vector3& generate_location , const GameLib::Vector3& size)
	{
		// 生成するゲームオブジェクトクラスを動的確保
		T* new_instance = new T();
		// GameObjectBaseを継承しているか確認
		Object* new_object = dynamic_cast<Object*>(new_instance);
		// エラーチェック
	...
	void DestroyAllObject()
	{
		...
		for (Object* obj : game_objects_list)
		{
			obj->Finalize();
			delete obj;
		}
