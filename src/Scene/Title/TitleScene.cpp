...
void TitleScene::Initialize()
{
    ...
    image = rm->GetImages("Resource/Images/title_space.png")[0];
    images = rm->GetImages("Resource/Images/UI/select_2.png", 6, 6, 1, 150, 150);
    sounds.push_back(rm->GetSounds("Resource/Sounds/BGM/Title.mp3", true));
...
SceneType TitleScene::Update(double deltaTime)
{
...
void TitleScene::Draw()
{
    ...
    glPushMatrix();
    glTranslatef(INIT_POSX, 0, INIT_POSZ + (select_num * POSZ_SHIFT));
...
// 現在のシーンタイプ情報を取得する
const SceneType TitleScene::GetNowSceneType() const
{
    return SceneType::title;
}

void TitleScene::Anim_Select(double deltaTime)
{
    anim_time += deltaTime;
    if (anim_time >= 0.1)
    {
...
void TitleScene::DrawBillboardQuad(GLuint tex, float size)
{
